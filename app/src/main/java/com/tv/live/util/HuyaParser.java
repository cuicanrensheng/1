...
public class HuyaParser {
    ...
    private static final String API_STREAM_INFO = "https://www.huya.com/cache.php?m=Live&do=getLiveInfo&roomId=%d";
    ...
    private static final String API_LIVE_PLAY_INFO = "https://www.huya.com/cache.php?m=Live&do=getLiveInfo&roomId=%d";
    private static final String API_GET_STREAM = "https://mdn.huya.com/api/v1/channel/%d";
    private static final String API_ROOM_REPLAY = "https://www.huya.com/room/%d";
    private static final String API_ALD_PLAY = "https://ald.huya.com/?roomid=%d&appid=5002&sdkver=4.6.0&stream=1";
            ...
            try {
                ...
                if (!TextUtils.isEmpty(pcHtml)) {
                    ...
                    if (!TextUtils.isEmpty(result[0])) {
                        hlsUrl = result[0];
                        Log.d("HuyaParser", "从PC网页获取到hls：" + hlsUrl);
                    }
                    if (!TextUtils.isEmpty(result[1])) {
                        flvUrl = result[1];
                        Log.d("HuyaParser", "从PC网页获取到flv：" + flvUrl);
                    }
                }
                ...
                if (TextUtils.isEmpty(hlsUrl) && TextUtils.isEmpty(flvUrl)) {
                    ...
                    if (!TextUtils.isEmpty(mobileHtml)) {
                        String[] result = extractUrlsFromHtml(mobileHtml);
                        if (!TextUtils.isEmpty(result[0])) {
                            hlsUrl = result[0];
                            Log.d("HuyaParser", "从移动端网页获取到hls：" + hlsUrl);
                        }
                        if (!TextUtils.isEmpty(result[1])) {
                            flvUrl = result[1];
                            Log.d("HuyaParser", "从移动端网页获取到flv：" + flvUrl);
                        }
                    }
                }
                ...
                if (TextUtils.isEmpty(hlsUrl) && TextUtils.isEmpty(flvUrl)) {
                    Log.d("HuyaParser", "尝试从LivePlayInfo API获取播放地址");
                    String result = fetchFromLivePlayInfoAPI(roomId);
                    if (!TextUtils.isEmpty(result)) {
                        if (result.endsWith(".m3u8")) {
                            hlsUrl = result;
                        } else {
                        ...
                        Log.d("HuyaParser", "从LivePlayInfoAPI获取到地址：" + result);
                    }
                }
    ...
    private static String fetchFromStreamAPI(int roomId) {
        try {
            String url = String.format(Locale.ROOT, API_ALD_PLAY, roomId);
            
            Map<String, String> headers = new HashMap<>();
            headers.put("User-Agent", "HYSDK/4.6.0 (Android; 30)");
            ...
            Response response = NetUtil.getInstance().syncGetWithHeaders(url, headers);
            if (!response.isSuccessful() || response.body() == null) {
                Log.d("HuyaParser", "fetchFromStreamAPI请求失败，状态码：" + response.code());
                return "";
            }
            
            String jsonStr = response.body().string();
            Log.d("HuyaParser", "StreamAPI返回长度：" + jsonStr.length());
            
            if (jsonStr.contains("<!DOCTYPE") || jsonStr.contains("<html")) {
                Log.d("HuyaParser", "StreamAPI返回的是HTML，不是JSON");
                return "";
            }
            
            Log.d("HuyaParser", "StreamAPI前300字符：" + jsonStr.substring(0, Math.min(300, jsonStr.length())));
            
            try {
                JSONObject json = new JSONObject(jsonStr);
                int status = json.optInt("status", -1);
                int code = json.optInt("code", -1);
                if (status != 200 && status != 0 && code != 200 && code != 0) {
                    Log.d("HuyaParser", "StreamAPI返回状态错误：status=" + status + ", code=" + code);
                    return "";
                }
                ...
                if (data == null) {
                    Log.d("HuyaParser", "StreamAPI无data字段");
                    return "";
                }
                
                Log.d("HuyaParser", "StreamAPI data keys: " + data.keys());
                
                if (data.has("hls_url") || data.has("HlsUrl") || data.has("hls")) {
                    String hls = data.optString("hls_url", "");
                    if (TextUtils.isEmpty(hls)) hls = data.optString("HlsUrl", "");
                    if (TextUtils.isEmpty(hls)) hls = data.optString("hls", "");
                ...
                JSONArray streamList = data.optJSONArray("streamList");
                if (streamList != null && streamList.length() > 0) {
                    JSONObject firstStream = streamList.getJSONObject(0);
                    if (firstStream.has("hlsUrl")) {
                        return firstStream.getString("hlsUrl");
                    }
                    if (firstStream.has("flvUrl")) {
                        return firstStream.getString("flvUrl");
                    }
                }
                
                JSONArray vStreamList = data.optJSONArray("vStreamList");
                if (vStreamList != null && vStreamList.length() > 0) {
                    JSONObject firstStream = vStreamList.getJSONObject(0);
                    if (firstStream.has("sHlsUrl")) {
                        String sHlsUrl = firstStream.getString("sHlsUrl");
                        if (firstStream.has("sHlsAntiCode")) {
                            sHlsUrl = sHlsUrl + "?" + firstStream.getString("sHlsAntiCode");
                        }
                        return sHlsUrl;
                    }
                    if (firstStream.has("sFlvUrl")) {
                        String sFlvUrl = firstStream.getString("sFlvUrl");
                        if (firstStream.has("sFlvAntiCode")) {
                            sFlvUrl = sFlvUrl + "?" + firstStream.getString("sFlvAntiCode");
                        }
                        return sFlvUrl;
                    }
                }
                ...
                Log.d("HuyaParser", "解析StreamAPI失败：" + e.getMessage());
                e.printStackTrace();
            ...
            Log.d("HuyaParser", "fetchFromStreamAPI异常：" + e.getMessage());
        }
        return "";
    }
    ...
    private static String fetchFromLivePlayInfoAPI(int roomId) {
        try {
            String url = String.format(Locale.ROOT, API_LIVE_PLAY_INFO, roomId);
            ...
            Response response = NetUtil.getInstance().syncGetWithHeaders(url, headers);
            if (!response.isSuccessful() || response.body() == null) {
                Log.d("HuyaParser", "fetchFromLivePlayInfoAPI请求失败，状态码：" + response.code());
                return "";
            }
            
            String jsonStr = response.body().string();
            Log.d("HuyaParser", "LivePlayInfoAPI返回长度：" + jsonStr.length());
            ...
            try {
                ...
                if (json.has("data")) {
                    ...
                    if (data.has("gameLiveInfo")) {
                        ...
                        if (liveInfo.has("liveStreamInfo")) {
                            JSONObject streamInfo = liveInfo.getJSONObject("liveStreamInfo");
                            if (streamInfo.has("sHlsUrl")) {
                                String sHlsUrl = streamInfo.getString("sHlsUrl");
                                if (streamInfo.has("sHlsAntiCode")) {
                                    sHlsUrl = sHlsUrl + "?" + streamInfo.getString("sHlsAntiCode");
                                }
                                return sHlsUrl;
                            }
                            if (streamInfo.has("sFlvUrl")) {
                                String sFlvUrl = streamInfo.getString("sFlvUrl");
                                if (streamInfo.has("sFlvAntiCode")) {
                                    sFlvUrl = sFlvUrl + "?" + streamInfo.getString("sFlvAntiCode");
                                }
                                return sFlvUrl;
                            }
                        }
                ...
                Log.d("HuyaParser", "解析LivePlayInfoAPI失败：" + e.getMessage());
                String urlFromJson = extractUrlFromJsonString(jsonStr);
                if (!TextUtils.isEmpty(urlFromJson)) {
                    return urlFromJson;
                }
            ...
            Log.d("HuyaParser", "fetchFromLivePlayInfoAPI异常：" + e.getMessage());
        }
        return "";
    }
    ...
    private static String[] extractUrlsFromHtml(String html) {
        ...
        try {
            ...
            Log.d("HuyaParser", "extractUrlsFromHtml异常：" + e.getMessage());
        }
