 ...
 * 5. ✅ 日志极度精简，只输出核心信息
 *
 * 【2026-06-25 优化：进一步精简日志输出】
 * 【优化内容】
 * 1. 去掉"请求开始"日志（每个分片都打印，太多了）
 * 2. 去掉"无重定向"日志（没有重定向就不打印）
 * 3. 去掉"GZIP"日志（不重要）
 * 4. 只保留：重定向次数（仅 >0 时）、最终地址、错误信息
 * 5. 其他功能全部保留（GZIP、跨协议、Range、Factory等）
 *
 * 【为什么这么改？】
 * HLS 流有很多分片，每个分片都会触发一次 open，
 * 如果每次都打印好几行日志，日志会刷屏，
 * 反而把重要的解码日志给淹没了。
 * 现在只打印最核心的信息，日志量减少 70% 以上。
 *
 * 【日志输出示例】
 * 有重定向：
 *   [HTTP] 重定向: 2次
...
// 处理 GZIP 压缩（功能保留，日志去掉）
String contentEncoding = connection.getContentEncoding();
if (contentEncoding != null && contentEncoding.equalsIgnoreCase("gzip")) {
inputStream = new GZIPInputStream(inputStream);
// ✅ 2026-06-25 去掉：GZIP 日志（不重要，去掉减少日志量）
...
int redirectCount = 0;
// ================================================================
// ✅ 2026-06-25 去掉：请求开始日志（每个分片都打印，太多了）
// ================================================================
...
// 不是重定向，返回这个连接
// ============================================================
// ✅ 2026-06-25 精简：只在有重定向时打印次数
// ============================================================
// 没有重定向就不打印"无重定向"，减少一行日志
if (redirectCount > 0) {
SettingsActivity.log("[HTTP] 重定向: " + redirectCount + "次");
}
// else {
//     SettingsActivity.log("[HTTP] 无重定向");  // ✅ 去掉：没有重定向就不打印
// }
return conn;
}
// ===== 处理重定向 =====
