...
public class TVPlayerManager {
    ...
    private void initPlayerListener() {
        playerListener = new Player.Listener() {
            @Override public void onPlayerError(PlaybackException error) { Log.e(TAG, "播放异常: " + error.getMessage()); if (listener != null) listener.onPlayError(error.getMessage()); autoRetry("播放错误：" + error.getMessage()); }
            @Override public void onPlaybackStateChanged(int state) {
                if (state == Player.STATE_READY) {
                    updateWakeLock(true); notifyLiveInfoUpdate(); showChannelAndAutoHide();
                    if (listener != null) listener.onPlayReady(); retryCount = 0; isRetrying = false; startStuckDetection();
                    if (initialPlayStartTime == 0) initialPlayStartTime = System.currentTimeMillis();
                    if (mDecoderMode == DECODER_MODE_AUTO && !hasSwitchedDecoder && initialPlayStartTime > 0 && System.currentTimeMillis() - initialPlayStartTime < 15000 && bufferCount > 1) {
                        hasSwitchedDecoder = true; setDecoderMode(DECODER_MODE_SOFT);
                    }
                } else if (state == Player.STATE_BUFFERING) { if (listener != null) listener.onBuffering(); lastPositionUpdateTime = System.currentTimeMillis(); bufferCount++; }
                else if (state == Player.STATE_ENDED) autoRetry("播放结束");
                else if (state == Player.STATE_IDLE) updateWakeLock(false);
            }
            @Override public void onIsPlayingChanged(boolean isPlaying) { }
            @Override public void onVideoSizeChanged(VideoSize videoSize) { notifyLiveInfoUpdate(); }
        };
        player.addListener(playerListener);
    }

    private void startStuckDetection() { stuckHandler.removeCallbacks(stuckCheckRunnable); lastPositionUpdateTime = System.currentTimeMillis(); lastPosition = 0; stuckHandler.postDelayed(stuckCheckRunnable, 2000); }
    private void stopStuckDetection() { stuckHandler.removeCallbacks(stuckCheckRunnable); }
    private void cancelRetry() { if (retryRunnable != null) { mHandler.removeCallbacks(retryRunnable); retryRunnable = null; } isRetrying = false; }
    ...
    private void autoRetry(String reason) {
        ...
        isRetrying = true; retryCount++;
        retryRunnable = new Runnable() { @Override public void run() { isRetrying = false; if (!TextUtils.isEmpty(currentUrl)) playUrlInternal(currentUrl); retryRunnable = null; } };
        mHandler.postDelayed(retryRunnable, 1000);
    }

    public void setDecoderMode(int mode) { /* ...原样保留... */ }
    public int getDecoderMode() { return mDecoderMode; }
    @Deprecated public void setSoftwareDecoder(boolean useSoftware) { setDecoderMode(useSoftware ? DECODER_MODE_SOFT : DECODER_MODE_AUTO); }

    public void registerDecoderModeReceiver() { /* ...原样保留... */ }
    public void unregisterDecoderModeReceiver() { /* ...原样保留... */ }

    // ========================================================================
    // 🚀 路线B的终极方案：通过 XML 样式动态重建视图
    // ========================================================================
    private void switchRenderer(boolean useTexture) {
        ...
        // ✅ 核心逻辑：根据开关选择对应的 Style 资源
        int styleRes = useTexture ? R.style.PlayerView_Texture : R.style.PlayerView_Surface;

        PlayerView newPlayerView = new PlayerView(context, null, 0, styleRes);
        ...
        // ✅ 触发回调，通知 MainActivity 重新绑定手势和焦点！
    ...
    public void onForeground() { try { if (player != null && playerView != null) { playerView.setPlayer(player); player.play(); } } catch (Exception e) { Log.e(TAG, "切前台异常", e); } }
    public void onBackground() { try { if (player != null) player.pause(); } catch (Exception e) { Log.e(TAG, "切后台异常", e); } }
    ...
    private void updateWakeLock(boolean enable) { isPlaying = enable; if (playerView != null) playerView.setKeepScreenOn(enable); }
    private String getLogTime() { return "[" + logSdf.format(new Date()) + "]"; }
    ...
    public void playUrl(String url, String channelName) {
        ...
        cancelRetry(); retryCount = 0; isRetrying = false; hasSwitchedDecoder = false; initialPlayStartTime = 0; resetPerformanceStats();
    ...
    private void resetPerformanceStats() { bufferCount = 0; totalStallTime = 0; isStalled = false; lastStallStartTime = 0; }
    ...
    private void playUrlInternal(String url) {
        try {
            ...
            MediaSource mediaSource = currentUrl.toLowerCase().contains("m3u8") ? new HlsMediaSource.Factory(httpFactory).createMediaSource(mediaItem) : new ProgressiveMediaSource.Factory(httpFactory).createMediaSource(mediaItem);

            player.setMediaSource(mediaSource, true);
            player.prepare(); player.play(); startStuckDetection();
        } catch (Exception e) { Log.e(TAG, "播放异常", e); autoRetry("播放异常：" + e.getMessage()); }
    }
    ...
    public static class LiveInfo { public String resolution = "未知"; public String bitrate = "0"; public String audio = "未知"; public String format = "未知"; }
    public LiveInfo getLiveInfo() { /* ... 返回 LiveInfo ... */ return new LiveInfo(); }
    private void notifyLiveInfoUpdate() { if (liveInfoUpdateListener != null) liveInfoUpdateListener.onLiveInfoUpdate(getLiveInfo()); }
    ...
    public void pause() { try { if (player != null) player.pause(); } catch (Exception e) { Log.e(TAG, "暂停异常", e); } }
    public void resume() { try { if (player != null) player.play(); } catch (Exception e) { Log.e(TAG, "恢复异常", e); } }

    public void release() {
        try {
            stopStuckDetection(); cancelRetry(); mHandler.removeCallbacks(hideChannelRunnable); updateWakeLock(false);
            unregisterDecoderModeReceiver(); unregisterRendererModeReceiver();
            if (player != null) { if (playerListener != null) player.removeListener(playerListener); player.release(); player = null; }
            instance = null;
        } catch (Exception e) { Log.e(TAG, "释放异常", e); }
    }
    ...
    private static class SoftwareFirstMediaCodecSelector implements MediaCodecSelector {
        ...
        @Override public List<MediaCodecInfo> getDecoderInfos(String mimeType, boolean requiresSecureDecoder, boolean requiresTunnelingDecoder) throws MediaCodecUtil.DecoderQueryException {
            ...
            switch (decoderMode) {
                case DECODER_MODE_HARD: { List<MediaCodecInfo> hardCodecs = new ArrayList<>(); for (MediaCodecInfo codec : allCodecs) { if (!isSoftwareDecoder(codec.name)) hardCodecs.add(codec); } return hardCodecs; }
                case DECODER_MODE_SOFT: { List<MediaCodecInfo> softCodecs = new ArrayList<>(); List<MediaCodecInfo> hardCodecs2 = new ArrayList<>(); for (MediaCodecInfo codec : allCodecs) { if (isSoftwareDecoder(codec.name)) softCodecs.add(codec); else hardCodecs2.add(codec); } List<MediaCodecInfo> result = new ArrayList<>(); result.addAll(softCodecs); result.addAll(hardCodecs2); return result; }
